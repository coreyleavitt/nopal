...
when defined(aarch64):
  ...
  switch("passL", "--target=aarch64-linux-musl --sysroot=/opt/musl/aarch64 -static -fuse-ld=lld")
...
when defined(armv7hf):
  ...
  switch("passL", "--target=armv7-linux-musleabihf --sysroot=/opt/musl/armv7hf -static -fuse-ld=lld")
...
when defined(mips):
  ...
  switch("passL", "--target=mips-linux-musl --sysroot=/opt/musl/mips -static -fuse-ld=lld")
...
when defined(mipsel):
  ...
  switch("passL", "--target=mipsel-linux-musl --sysroot=/opt/musl/mipsel -static -fuse-ld=lld")
